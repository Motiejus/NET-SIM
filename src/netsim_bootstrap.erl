...
%%
%% {Simulations, N} is awkward.
%% Purpose of the process is to run only the last simulation.
%% All elements but last of Simulations is a list of past simulations
%% that have to be loaded before the real simulation starts. Ugly hack, yes
-spec init(list(), list(), list(), list(), list(), list(), list(), list()) ->
    no_return().
init(NodesFiles, LinksFile, {Simulations, N}, SettingsFile, TicksFile,
    ...
    lists:foreach(
        fun netsim_serv:send_event/1,
        lists:sublist(Simulations, length(Simulations)-1)
    ),
    Simulation = hd(lists:reverse(Simulations)), % Execute this
